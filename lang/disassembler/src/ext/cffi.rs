//! CFFI interface for Move Disassembler.
//!
//! Build: `cargo build -p disassembler --lib --features cffi`
//!
//! Usage exapmle:
//!
...
//!     Result result = move_disassemble(bytes, length, true);
...
#[no_mangle]
#[export_name = "move_disassembler_ver"]
...
/// middle. __Ownership__ of result is yours, feel free to free.
#[no_mangle]
#[export_name = "move_disassemble"]
pub unsafe extern "C" fn disassemble(bytes: *const u8, len: usize, compat_mode: bool) -> Result {
    let mut bytes = {
        if !bytes.is_null() {
    ...
    if compat_mode {
        match compat::adapt(&mut bytes).map_err(map_err) {
            Ok(_) => {}
            Err(err) => {
                return Result {
                    error: err.into_raw(),
    ...
    let out = match disasm_str(&bytes, cfg).map_err(map_err) {
        Ok(v) => v,
        Err(err) => {
            return Result {
                error: err.into_raw(),
